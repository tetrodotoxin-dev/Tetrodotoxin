...
// Constant evaluation may build a temporary self-reference. Only the finished
// bytes escape preparation, so no constexpr allocation or source pointer is
// retained in the publication.
...
PERIMORTEM_UNIT_TEST(Pointers, recursive_callable) {
  ...
  const auto callback = Schema::callable(Schema::Abi::SystemVAMD64, {&self, 1});
