...
class Foreign {
 public:
  Foreign() {
    ...
    memcpy(name + 1, "libinterface_provider.so", sizeof("libinterface_provider.so"));
...
PERIMORTEM_UNIT_TEST(TtxSimulacra, retained_c_calls) {
  ...
  Simulacra::fulfill<Counter>(Query(foreign.api.query)).visit(
...
PERIMORTEM_UNIT_TEST(TtxSimulacra, refusal_boundaries) {
  ...
  for (const auto status : statuses) {
    ...
    Query(foreign.api.query).bind<Counter>().visit(
        [&](const Counter&) { EXPECT(False); },
        [&](Binding::Failure error) {
          EXPECT_EQ(static_cast<U8>(error), status == 99 ? TTX_BINDING_REJECTED : status);
        });
    EXPECT_EQ(foreign.api.statistics().queries, U64(1));
  ...
  Query(foreign.api.query).bind<Counter>().visit(
      [&](const Counter&) { EXPECT(False); },
      [&](Binding::Failure error) { EXPECT(error == Binding::Failure::Rejected); });
...
struct WrongResult { const void* receiver; R64 (*add)(const void*, U64); U64 (*read)(const void*); };
struct WrongArguments { const void* receiver; U64 (*add)(U64); U64 (*read)(const void*); };
struct WrongAbi { const void* receiver; U64 (*add)(const void*, U64, ...); U64 (*read)(const void*); };
struct WrongOrder { const void* receiver; U64 (*read)(const void*); U64 (*add)(const void*, U64); };
TTX_DATA_RECORD(WrongResult, TTX_DATA_MEMBER(WrongResult,receiver), TTX_DATA_MEMBER(WrongResult,add), TTX_DATA_MEMBER(WrongResult,read));
TTX_DATA_RECORD(WrongArguments, TTX_DATA_MEMBER(WrongArguments,receiver), TTX_DATA_MEMBER(WrongArguments,add), TTX_DATA_MEMBER(WrongArguments,read));
TTX_DATA_RECORD(WrongAbi, TTX_DATA_MEMBER(WrongAbi,receiver), TTX_DATA_MEMBER(WrongAbi,add), TTX_DATA_MEMBER(WrongAbi,read));
TTX_DATA_RECORD(WrongOrder, TTX_DATA_MEMBER(WrongOrder,receiver), TTX_DATA_MEMBER(WrongOrder,read), TTX_DATA_MEMBER(WrongOrder,add));
...
PERIMORTEM_UNIT_TEST(TtxSimulacra, mismatched_callables) {
  ...
  for (const auto* form : forms) {
    ...
    const auto status = Query(foreign.api.query).bind(
        Counter::contract_id, Storage(ttx_storage{form, output, sizeof(output)}));
...
class Local {
 public:
  Local(Query fallback, Binding::Status status) : fallback(fallback), status(status) {}
...
PERIMORTEM_UNIT_TEST(TtxSimulacra, native_policy) {
  ...
  for (auto status : statuses) {
    ...
    Simulacra::fulfill<Counter>(native).visit(
        [&](const Counter& counter) {
          EXPECT(status == Binding::Status::Satisfied || status == Binding::Status::Unsupported);
          EXPECT_EQ(counter.add(9), U64(9));
        },
        [&](Binding::Failure failure) {
          EXPECT_EQ(static_cast<U8>(failure), static_cast<U8>(status));
        });
    EXPECT_EQ(foreign.api.statistics().queries,
